...
function fegu() {
    ...
    for (x = post; x < 5; x++) {
        ...
        huddle.push(new circles(canvas.width / 2, y - 950 - x * 400, 0, 0.02 + x * 0.005 , true, true, clock));
    }
...
function run() {
    ...
    if (loop == true) {
        ...
        createhuddle_();
        jump();
        let bangoo = localStorage.getArray('hackonama');
        ...
        ctx.drawImage(background, 0, 0);
        localStorage.pushArrayItem('hackonama', score);
        let isdesh = localStorage.getArray('hackonama');
