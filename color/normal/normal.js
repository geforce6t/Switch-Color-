...
function fegu() {
    ...
    for (x = post; x < 5; x++) {
        huddle.push(new circles(canvas.width / 2, y - 1180 - x * (canvas.height/1.5), 0, 0.04 + x * 0.0005, true, true));
    }
...
function run() {
    ...
    if (loop == true) {
        ...
        createhuddle_();
        jump();
        let bangoo = localStorage.getArray('hackonab');
        ...
        ctx.drawImage(background, 0, 0);
        localStorage.pushArrayItem('hackonab', score);
        let isdesh = localStorage.getArray('hackonab');
